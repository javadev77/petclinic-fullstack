...
public class JpaOwnerRepositoryImpl implements OwnerRepository {
    ...
    public PageImpl findAll(Pageable pageable) {
        // 1️⃣ requête principale (data)
        String jpql = "SELECT owner FROM Owner owner";

        // gestion du tri
        if (pageable.getSort().isSorted()) {
            String orderBy = pageable.getSort().stream()
                .map(o -> "owner." + o.getProperty() + " " + o.getDirection())
                .reduce((a, b) -> a + ", " + b)
                .orElse("");
    ...
    public Page<Owner> findPageByLastName(String lastName, Pageable pageable) {
        ...
        long total = countByLastName(lastName);
        return new PageImpl<>(owners, pageable, total);
    }

    @Override
    public long countByLastName(String lastName) {
        Query query = this.em.createQuery("SELECT COUNT(owner) FROM Owner owner WHERE owner.lastName LIKE :lastName");
        query.setParameter("lastName", lastName + "%");
